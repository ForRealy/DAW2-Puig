...
function analitzarTemperatures(...valores) {
    const suma = valores.reduce((acumulador, actual) => acumulador + actual, 0);
    const media = (suma / valores.length).toFixed(2);
    const table = document.getElementById('taulaTemperatures');
    if (table) {
...
function enviarMissatges(dat, msj, salu) {
    if (typeof dat == "string") {
        if (dat.includes("@")) {
            console.log(dat, msj, salu);
        }
        else
            alert("L'email no té un format vàlid");
    }
    if (typeof dat == "number") {
        ...
        if (datString.length == (9)) {
            ...
            alert("El telefon no té un format vàlid");
    }
}
